...
#define PIX_FMT AV_PIX_FMT_RGBA
    ...
    static int encode(AVCodecContext *enc_ctx, AVPacket *pkt, AVFrame *frame, AVFormatContext *fmt, AVStream *stream)
    {
        int ret = 0;
        if (frame)
        {
    ...
    int encode_rgba(std::string input_file, std::string output_file)
    {
        ...
        enc_ctx->rc_min_rate = BIT_RATE;
        enc_ctx->rc_max_rate = BIT_RATE;
        enc_ctx->bit_rate_tolerance = BIT_RATE;
        enc_ctx->rc_buffer_size = BIT_RATE;
        ...
        while (!feof(f_in))
        {
            ...
            fread(frame->data[0], 1, frame->width * frame->height * 4, f_in);
            frame->pts = frame_index++;
            encode(enc_ctx, pkt, frame, fmt_ctx, stream);
        }

        // 12. flush the encoder
        encode(enc_ctx, pkt, NULL, fmt_ctx, stream);
