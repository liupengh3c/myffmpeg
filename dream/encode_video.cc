...
#define PIX_FMT AV_PIX_FMT_YUV420P
    ...
    int encode_video(std::string input_file, std::string output_file)
    {
        ...
        if (!codec)
        {
            ...
            std::cout << "find encode cpu." << std::endl;
        }
        std::cout << "find encode gpu hardware." << std::endl;
        // 2. 分配编码器上线问
        ...
        std::cout << "encode yuv(1280*720) duration: " << frame_index / 25 << ",cost time:" << iend - istart << std::endl;
        return 0;
    }
}